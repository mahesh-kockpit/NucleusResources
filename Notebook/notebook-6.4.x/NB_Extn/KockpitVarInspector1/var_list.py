...
def _getsizeof(x):
    ...
    if type(x).__name__ in ['ndarray', 'Series']:
        ...
        return x.memory_usage().sum() if hasattr(x,'memory_usage') else -1
    else:
        return getsizeof(x)
...
def var_dic_list():
    ...
    '''
    vardic = [{'varName': v, 'varType': type(eval(v)).__name__, 'varSize': str(_getsizeof(eval(v))), 'varShape': str(_getshapeof(eval(v))) if _getshapeof(eval(v)) else '', 'varContent': str(eval(v).columns).replace("'","") if fullname(eval(v)) == 'pyspark.sql.dataframe.DataFrame' else str(eval(v))[:200]}  # noqa
    for v in values if (v not in ['_html', '_nms', 'NamespaceMagics', '_Jupyter']) & (type(eval(v)).__name__ not in types_to_exclude)] # noqa 
